...
/*     AccountADC();
     ...
     {
       EEPROM_write_word(count_channel, array[count_channel]); //записать калибровочную константу
...
*/
  ...
  else                                  //проверка наличия констант в EEPROM:      
  {
/*    unsigned char i;
    unsigned int sum = 0;

    for (i=0; i<9; i++)                 //считывание массива уставок из EEPROM
    {
      array[i] = EEPROM_read_word(i);
      sum += array[i];
    }

    if (sum==0)                         //если уставки равны нулю
    {
      for (i=0; i<9; i++)               //записать рассчитанные уставки в EEPROM
        EEPROM_write_word(i, ustavki[i]);
      array[i] = ustavki[i];            //записать рассчитанные уставки в ОЗУ
    }
*/  
    /*Work program*/
    while (1)
